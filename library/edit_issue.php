...
$stmt = $conn->prepare('SELECT book_issue.*, students.name, students.department, library_books.book_name, library_books.author FROM book_issue INNER JOIN students ON students.student_id = book_issue.student_id INNER JOIN library_books ON library_books.book_id = book_issue.book_id WHERE book_issue.issue_id = ?');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($issueDate === '' || $expectedReturnDate === '') {
        ...
        try {
            ...
            $updateStmt = $conn->prepare('UPDATE book_issue SET issue_date = ?, expected_return_date = ?, actual_return_date = ?, status = ? WHERE issue_id = ?');
...
<section class="page-heading">
    <div>
        ...
        <p>Update due dates and complete the return workflow without losing history.</p>
    </div>
</section>
